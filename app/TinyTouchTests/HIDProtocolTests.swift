...
final class HIDProtocolTests: XCTestCase {
    ...
    func testMalformedBadMACReplayAndParsers() throws {
        ...
        let status = try DeviceStatus(line: "OK STATUS firmware_version=1.2 protocol=2 mode=hid sensor=ok fingerprints=3 fingerprint_slots=1,3,5 hid_hosts=2")
    ...
    func testProtocolSixDialectStatusAliasesAndUnknownVersion() throws {
        ...
        XCTAssertTrue(status.isProvisioned(mode: .hid))
        XCTAssertFalse(status.isProvisioned(mode: .piv))
        XCTAssertTrue(status.isSetupComplete(mode: .hid))
        let piv = try DeviceStatus(line: "OK STATUS protocol=6 mode=piv piv=ready sensor=ready fingerprints=4 hosts=0")
        XCTAssertTrue(piv.isProvisioned(mode: .piv)); XCTAssertTrue(piv.isSetupComplete(mode: .piv))
    ...
    func testPIVPairingFailureKeepsProvisioningReady() {
        ...
        XCTAssertNil(SCAuthResult.pairingLaunch(exitCode: 1, error: "cancelled"))
        XCTAssertNotNil(SCAuthResult.pairingLaunch(exitCode: -1, error: "could not start"))
