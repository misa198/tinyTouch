...
static void status(void) {
  ...
  if (slot_mask >= 0) {
    ...
    snprintf(count, sizeof(count), "%d", enrolled);
  }
  snprintf(line, sizeof(line),
           "OK STATUS product_id=misa198.tinytouch.v1 protocol=6 firmware=%s build=%s mode=%s piv=%s sensor=%s fingerprints=%s "
